...
namespace CyanobotsGenes
{
    ...
    class CG_Init
    {
        ...
        public static void ApplySettingsToDefs()
        {
            ...
            DefDatabase<GeneDef>.ClearCachedData();
            */
            /*
            List<ThingDef> mealDefs = new List<ThingDef>
            {
                
                DefDatabase<ThingDef>.GetNamed("MealSurvivalPack"),
                DefDatabase<ThingDef>.GetNamed("MealSimple"),
                DefDatabase<ThingDef>.GetNamed("MealFine"),
                DefDatabase<ThingDef>.GetNamed("MealFine_Veg"),
                DefDatabase<ThingDef>.GetNamed("MealLavish"),
                DefDatabase<ThingDef>.GetNamed("MealLavish_Veg")
            };

            foreach (ThingDef mealDef in mealDefs)
            {
                Log.Message("Found def " + mealDef.defName);
                if (mealDef.GetType().GetField("cachedGraphic", BindingFlags.Instance | BindingFlags.NonPublic) != null)
                {
                    mealDef.GetType().GetField("cachedGraphic", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(mealDef, null);
                }
            }
            */
        }
