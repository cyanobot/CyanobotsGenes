		...
		public Toil FinalizeIngest()
		{
			...
			toil.initAction = delegate
			{
                ...
                else
				{
                    ...
                    {
						//numToIngest = Math.Min(thing.def.ingestible.maxNumToIngestAtOnce, job.count);
						//Log.Message("job.count: " + job.count + ", maxNumToIngestAtOnce: " + thing.def.ingestible.maxNumToIngestAtOnce);
						effectiveNutritionWanted = thing.GetStatValue(StatDefOf.Nutrition) * stackCount;
					}
