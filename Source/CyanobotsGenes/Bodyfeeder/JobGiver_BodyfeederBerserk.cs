...
namespace CyanobotsGenes
{
    class JobGiver_BodyfeederBerserk : ThinkNode_JobGiver
    {
        ...
        {
            Job jobIngest = BodyfeederUtility.MakeIngestForHemogenJob(pawn, 40f);
            if (jobIngest != null) return jobIngest;
        ...
        private Pawn FindPawnTarget(Pawn pawn)
        {
            return (Pawn)AttackTargetFinder.BestAttackTarget(pawn, TargetScanFlags.NeedReachable, 
                (Thing x) => x is Pawn pawn2 && pawn2.Spawned && !pawn2.Downed && !pawn2.IsPsychologicallyInvisible()
